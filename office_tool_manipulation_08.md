...
前置きはここまでにして、実際に`AND`関数を使ってみましょう。現在、アイテムシートの攻撃力と守備力には、`*`演算子を使った条件付き書式が設定されているはずです。これを、`AND`関数で置き換えてみます。
...
このように、`AND`関数と`*`演算子は、どちらも「２つの条件を組み合わせる」機能を持ちます。ですが、`*`演算子には「掛け算」と「組み合わせ」の両方の機能があるのに対して、`AND`関数の機能は「組み合わせ」だけです。

目的に特化した機能を使うほうが、あとで条件を見たときにも理解しやすいので、基本的には`AND`関数を使うほうがよいでしょう。
...
「複数の条件が、少なくとも１つ満たされる」ことを知りたい場合があります。例えば、「店の種類が`鍵`、または`聖水`、または`宿`のとき」を調べたい場合です。これには以下の２つの方法があります。
...
<pre class="tnmai_assignment">
<strong>【課題７】</strong>
「敵の分析」シートにある「補助魔法を使う敵」と「攻撃魔法・特殊攻撃を使う敵」の２つの表について、
表として見やすくなるように、フォントや背景色を変更したり、枠線を設定しなさい。
</pre>
