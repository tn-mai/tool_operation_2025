...
条件付き書式や入力規則を使うと、入力ミスを防ぐことができます。しかし、「適切な成長速度になっているか」とか「敵の強さはレベル帯に合っているか」のような、「データ全体を俯瞰しなくては確認できない情報」は、これらの「入力をチェックする」方法では調べられません。
...
3. E2～E5セルに次の文章を入力
    ...
    * 通常攻撃+攻撃魔法・特殊攻撃
    * 全て
4. F2セルに`=SUM((モンスターデータ!$H$2:$H$41="-")*(モンスターデータ!$J$2:$J$41="-"))`という数式を入力
5. F3セルに`=SUM((モンスターデータ!$H$2:$H$41<>"-")*(モンスターデータ!$J$2:$J$41="-"))`という数式を入力
6. F4セルに`=SUM((モンスターデータ!$H$2:$H$41="-")*(モンスターデータ!$J$2:$J$41<>"-"))`という数式を入力
7. F5ｾﾙに`=SUM((モンスターデータ!$H$2:$H$41<>"-")*(モンスターデータ!$J$2:$J$41<>"-"))`という数式を入力
...
同じことは`ROUNDDOWN`関数でもできますが、`INT`関数のほうが名前が短く、桁数の指定も不要なので、数式が読みやすくなるのが利点です。
