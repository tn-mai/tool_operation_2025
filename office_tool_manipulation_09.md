...
条件付き書式や入力規則を使うと、入力ミスを防ぐことができます。しかし、「適切な成長速度になっているか」とか「敵の強さはレベル帯に合っているか」のような、「データ全体を俯瞰しなくては確認できない情報」は、<br>
これらの「入力をチェックする」方法では調べられません。

かといって、「じっくりと表をながめて適切なデータかどうかを確認する」というのでは時間がかかりすぎますし、見つけられる保証もありません。

「グラフ」は、このような場合に役立ちます。<br>
グラフを使って情報を視覚化すると、数値を見ているだけでは気づかないような問題を見つけられます。
...
作成されたグラフがおおよそ直線になっていれば、うまくデータを入力できています。<br>
大きく上下にぶれている部分があれば、その部分のデータは見直しが必要です。

### 1.2 グラフのタイトルを変更する

グラフの上部には、グラフのタイトルが表示されます。<br>
タイトル部分をダブルクリックすると、タイトルテキストを変更できます。<br>
変更したら、`Enter`キーで入力を確定します。
...
グラフの作成でもっとも<ruby>厄介<rt>やっかい</rt></ruby>なのは、「適切な種類を選ぶ」ことです。<br>
主要なグラフの種類と用途を、以下の表に示します。
...
例えば、レベルアップ時のステータスが上がりすぎたり、上がらなさすぎたりしていないことを調べるには、<br>
「積み上げグラフ」が適しています。以下の手順で、積み上げグラフを作成してください。
...
さらに、よく見ると16レベルで大きく上昇するのは最大MPだけです。このグラフだけでは分かりませんが、17レベルになると最初の上位魔法を覚えます。上位魔法は消費MPが大きいため、ここで最大MPを増やすことで、上位魔法を使いやすくしている可能性が考えられます。

また、上位魔法の習得は、それだけで十分な成長を感じられます。新しい魔法を有効活用してもらうために、17レベルのステータス上昇量を少なめにしている、という見方もできるでしょう。実際に、18レベルでは大きくステータスが上昇します。

&emsp;**グラフは、データの傾向や入力ミスを見つけるのに役立ちます**
...
既存の表だけでは、作りたいグラフに必要なデータが見つからないことがあります。<br>
そんなときは、「グラフに必要なデータを持つ表」を作成します。

例えば、「遭遇表の番号が、遭遇マップに配置された数」を比較したいとします。<br>
ところが、残念なことに、そんなデータを持つ表は作っていないようです。

そこで、「遭遇表の番号ごとに、遭遇マップに配置された数」を示す表を追加します。

特定の値を数えるには、`COUNTIF`関数を使うと簡単です。また、数列を作るには`SEQUENCE`
(シーケンス)関数が便利です。この2つの関数を使って表の元になるデータを作成しましょう。<br>
次の手順にしたがって、遭遇マップに配置された番号の数を計算してください。
...
&emsp;**SEQUENCE(行数, 列数, 最初の数値, 増分)**

「行数」以外のパラメータは省略可能です。省略すると`1`が指定されたとみなされます。<br>
先に書いた`SEQUENCE(14)`の場合、次のような14行1列の数列が作成されるわけです。
...
&emsp;**既存のデータからでは適切なグラフを作成できない場合、「グラフ用の表を作成」します**
...
3. E2～E5セルに次の文章を入力
    ...
    * 通常攻撃+攻撃魔法・特殊攻撃
    * 全て
4. F2セルに`=SUM((モンスターデータ!$H$2:$H$41="-")*(モンスターデータ!$J$2:$J$41="-"))`<br>という数式を入力
5. F3セルに`=SUM((モンスターデータ!$H$2:$H$41<>"-")*(モンスターデータ!$J$2:$J$41="-"))`<br>という数式を入力
6. F4セルに`=SUM((モンスターデータ!$H$2:$H$41="-")*(モンスターデータ!$J$2:$J$41<>"-"))`<br>という数式を入力
7. F5セルに`=SUM((モンスターデータ!$H$2:$H$41<>"-")*(モンスターデータ!$J$2:$J$41<>"-"))`<br>という数式を入力
...
続いて、作成した表から円グラフを作ります。<br>
次の手順にしたがって、円グラフを作成してください。

1. E1:F5を範囲選択
2. 「挿入」タブにある「円」グラフから、「2D円」を選択
3. 「行動の種類」表の右側に円グラフを移動

<p align="center"><img width="50%" alt="2D円グラフ" src="images/excel_graph_circle.png" /></p>
...
3. 「グラフタイトルを中央揃えで重ねて配置」をクリック

<p align="center"><img width="50%" alt="タイトルの配置" src="images/excel_graph_circle_title_layout.png" /></p>
...
> 他にも、凡例を選択して`Delete`キーを押す、右クリックすると表示されるメニューから削除を選ぶ、<br>
> リボンから削除する、などの方法があります。
...
さて、データラベルとして最初に表示されるのは、円グラフの分割に使った数値です。ですが、数値が何を意味するのかは結局分からないので、凡例になりません。この部分は、凡例として役立つデータに変更する必要があります。次の手順にしたがって、データラベルの書式設定を行ってください。

1. グラフの右側にある`+`アイコンをクリック(Web版の場合はグラフをダブルクリック)
2. 「データ ラベル」項目の右にある`▶`アイコンから、「その他のオプション」を選択
3. 書式設定ウィンドウが表示されるので、以下の項目にチェックを入れて、それ以外の項目のチェックを外す
    * 分類名
    * パーセンテージ
    * 引き出し線を表示する
...
これで、データラベルが凡例の代わりになりました。<br>
データラベルが見づらい場合は、フォントの色を白などの淡い色に変えたり、太字にしてみるとよいでしょう。

<div style="page-break-after: always"></div>

### 2.2 敵が使う魔法の使用比率を円グラフにする

円グラフの練習として、４つの補助魔法ごとに使用する敵の総数を表にして、使用率をあらわす円グラフを作成します。次の手順にしたがって、円グラフに使う表を作成してください。

1. E11セルに「補助魔法名」、F11セルに「総数」と入力
2. E12セルに数式`=敵の行動!A3:A6`と入力
3. F12セルに数式`=SUM(INT($C$3:$C$17=E12))`と入力

`INT`(イント)関数は「数値を最も近い整数に切り下げる」関数です。<br>
`ROUNDDOWN`関数と似ていますが、次のような違いがあります。
...
ですが、今回`INT`関数を使っているのは切り下げが目的ではなく、`TRUE`と`FALSE`を数値の
`1`と`0`に変換するためです。`SUM`関数は数値しか受け付けないので、文字列の`TRUE`と
`FALSE`をそのまま渡すことができません。何らかの方法で数値に変換する必要があるのです。

同じことは`ROUNDDOWN`関数でも実現できます。<br>
ですが、`INT`関数のほうが名前が短く、桁数の指定も不要なので、数式が読みやすくなるのが利点です。
...
<pre class="tnmai_assignment">
<strong>【課題３】</strong>
４つの攻撃魔法・特殊攻撃ごとに、使用する敵の総数を表にして、
「<strong>攻撃魔法・特殊攻撃の使用比率</strong>」を視覚化する円グラフを作成しなさい。
タイトルが長いので、「中央揃えで重ねて配置」より「グラフの上」のほうが良いかもしれません。
</pre>

> このテキストで紹介したほかにも、Excelのグラフにはさまざまな表示方法や設定があります。<br>
> いろいろ試してみるとよいでしょう。
